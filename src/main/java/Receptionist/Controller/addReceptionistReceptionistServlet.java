...
@WebServlet("/receptionists/add-receptionist")
@MultipartConfig(
        fileSizeThreshold = 1024 * 1024 * 2,  // 2MB
        maxFileSize = 1024 * 1024 * 10,       // 10MB
        maxRequestSize = 1024 * 1024 * 50     // 50MB
)
public class addReceptionistReceptionistServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // 2. Map User Details
            User user = new User();user.setName(request.getParameter("name") != null ? request.getParameter("name").trim() : null);
            ...
            if (userId > 0) {
                ...
                response.sendRedirect(request.getContextPath() + "/Receptionist-dashboard?error=failed");
            }

        } catch (Exception e) {
            String message = java.net.URLEncoder.encode(e.getMessage() != null ? e.getMessage() : "Unable to save receptionist.", java.nio.charset.StandardCharsets.UTF_8);
